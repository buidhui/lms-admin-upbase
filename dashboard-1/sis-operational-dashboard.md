...
Tài liệu này hướng dẫn người dùng cách sử dụng và thao tác với SIS Operational Dashboard - nơi cung cấp các dashboard hỗ trợ theo dõi và phân tích dữ liệu vận hành học tập trên hệ thống.
...
| Course     | Y              | Không                | Program                                            | Hiển thị tên khóa học thuộc chương trình đã chọn. Nếu không chọn chương trình thì hiển thị toàn bộ khóa. Chỉ lấy course 4 level.        |
| From - To  | N              | Không                |                                                    | Từ ngày - Đến ngày. Nếu lọc theo thời gian → chỉ lấy các lớp có Start date nằm trong khoảng thời gian lọc.                              |
...
| Section    | N              | Không                | Chọn Khóa học, Lớp học trước mới được chọn Section | Hiển thị tên các section có trong khóa học đã chọn. Khi lọc theo filter này, các cột sẽ thể hiện các buổi học của lớp thuộc section đó. |
{% endstep %}
...
### Dashboard Phân phối điểm các bài kiểm tra
...
### Dashboard Tỉ lệ ghi điểm trên từng câu/chủ đề kiến thức
...
### Dashboard Tiến độ học trên LMS
