...
<figure><img src="https://sapp-academy.gitbook.io/files/FmPhRneWuSK0BHWnZBzB" alt=""><figcaption></figcaption></figure>

<figure><img src="https://sapp-academy.gitbook.io/files/EwveudAtZT4UQ1RKlvm0" alt=""><figcaption></figcaption></figure>
...
Tại tab **Student**, cột **Attendance** hiển thị theo dạng:

**\[Số buổi đã điểm danh / Tổng số buổi học]**

Ví dụ: 5/10 – Học viên đã điểm danh 5 buổi trên tổng số 10 buổi học.

<figure><img src="https://sapp-academy.gitbook.io/files/9NhQjxS4UEvnpdPKQMJx" alt=""><figcaption></figcaption></figure>
...
Click vào số liệu chuyên cần của học viên.
...
<figure><img src="https://sapp-academy.gitbook.io/files/MIDTQSO3e8SdaOegzdWW" alt=""><figcaption></figcaption></figure>
...
Tại bảng danh sách chuyên cần, click nút **Action** → chọn **"Attendance History"**.
...
Tại màn hình Class Detail, người dùng bấm nút **Action** tại học viên cần thực hiện điểm danh bù → chọn **Add Attendance**.
{% endstep %}

{% step %}
### Xem danh sách buổi học

Hệ thống hiển thị danh sách các buổi học viên có thể điểm danh bù.
{% endstep %}

{% step %}
### Chọn buổi học và thêm điểm danh

Người dùng tích chọn vào các buổi cần thực hiện điểm danh bù → ấn **Add**.
{% endstep %}

{% step %}
### Nhập lý do và lưu

Hệ thống hiển thị Pop-up nhập lý do điểm danh bù → người dùng nhập lý do → ấn **Save** để lưu.
{% endstep %}
{% endstepper %}
...
{% stepper %}
{% step %}
### Chọn Edit Attendance

Người dùng bấm nút **Action** tại buổi học cần chỉnh sửa → chọn **Edit Attendance**.
{% endstep %}

{% step %}
### Cập nhật và lưu thông tin điểm danh

Hệ thống hiển thị Pop-up **Edit Attendance** → người dùng chọn lại trạng thái điểm danh và lý do thay đổi → ấn **Save** để lưu.
{% endstep %}
{% endstepper %}
...
{% hint style="warning" %}
Ví dụ: Lịch học cố định buổi A từ 8:00 → 11:00, học viên vào link Zoom từ 7:30 → 11:30 → Hệ thống xác định khoảng 7:30 → 11:30 thuộc buổi A, sau đó tính tiếp thời lượng điểm danh.
{% endhint %}
...
<summary>Dữ liệu điểm danh từ Dahahi và Zoom có đồng bộ về LMS theo thời gian thực không?</summary>

Dữ liệu được đồng bộ tự động; thời gian cập nhật phụ thuộc vào cấu hình hệ thống.

</details>

<details>

<summary>Chức năng điểm danh bù và chỉnh sửa điểm danh có áp dụng cho lớp Dahahi không?</summary>

Hiện tại chức năng này chỉ áp dụng cho lớp học qua Zoom.

</details>

<details>

<summary>Khi học viên dùng nhiều thiết bị Zoom cùng lúc, hệ thống tính thế nào?</summary>

Hệ thống tự động hợp nhất thời gian và loại bỏ khoảng thời gian trùng lặp giữa các thiết bị.
