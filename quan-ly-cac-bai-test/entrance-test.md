...
{% stepper %}
{% step %}
## Vào danh sách bài kiểm tra

Vào **Menu → Entrance Test → Entrance Test List**.
{% endstep %}

{% step %}
## Xem thông tin danh sách
...
* (Tùy chọn) Tìm kiếm và lọc danh sách theo: **Tên bài kiểm tra**, **Program**, **Subject**, **Status**.
* Click vào tên bài kiểm tra để xem chi tiết gồm 2 tab:
...
1. Click **+ Add Section** để thêm section.
2. Điền thông tin section:
...
3. Click **Add Questions** để thêm câu hỏi vào section.
4. Lặp lại bước 4–6 để thêm các section khác nếu cần.
5. Click **Next** để chuyển sang bước 3.
{% endstep %}

{% step %}
## Tab 3 — Settings

1. Cài đặt Pass Point và Nhận xét cho từng Question Type:
...
2. Điều chỉnh điểm và nhận xét nếu cần.
...
{% hint style="warning" %}
**Quy tắc 1 Active/Locked mỗi Program:** Nếu muốn Active bài kiểm tra mới cùng Program → phải Deactivate bài đang Active/Locked trước.
{% endhint %}

**Quyền chỉnh sửa theo trạng thái:**

* Chỉnh sửa toàn bộ: Test Info, Sections (bao gồm danh sách câu hỏi), và Settings. Click vào tên bài kiểm tra → Click Edit. Chỉnh sửa thông tin cần thay đổi → Click Save.
* Quyền chỉnh sửa phụ thuộc vào việc đã có học viên đăng ký chưa: Click vào tên bài kiểm tra → Click Edit. Chỉnh sửa các trường được phép → Click Save.
* Chỉ được sửa một số trường giới hạn: Click vào tên bài kiểm tra → Click Edit. Chỉnh sửa các trường được phép → Click Save.
* Không thể chỉnh sửa bất kỳ thông tin nào. Đây là trạng thái cuối — không thể chuyển lại.
...
#### Xem danh sách học viên & chi tiết kết quả

**Kết quả:** Xem được danh sách học viên đã tham gia bài kiểm tra và kết quả chi tiết của từng người.

{% stepper %}
{% step %}
## Mở danh sách học viên

Vào **Entrance Test List** → click tên bài kiểm tra → chọn tab **Participants**.
{% endstep %}
...
1. (Tùy chọn) Tìm kiếm và lọc theo: **Tên học viên**, **Status**, **Level**, **Source**, **Sort by**, **Từ ngày / Đến ngày**.
2. Xem chi tiết kết quả: Click **Action → View Result** (hoặc click trực tiếp vào tên học viên).
{% endstep %}
{% endstepper %}
...
{% hint style="info" %}
**Trước khi bắt đầu:** Bài kiểm tra phải đang ở trạng thái **Active** hoặc **Locked**.
{% endhint %}

Có 2 cách thêm học viên thủ công:

{% tabs %}
{% tab title="Từ tab Participants" %}
1. Tại tab Participants của bài kiểm tra, click nút + Add Participants.
2. Tìm kiếm học viên theo tên hoặc email.
3. Chọn học viên cần thêm → Click Add.
{% endtab %}

{% tab title="Từ Action" %}
1. Tại tab Participants, click Action → Add Participants.
2. Tìm kiếm học viên theo tên hoặc email.
3. Chọn học viên cần thêm → Click Add.
{% endtab %}
{% endtabs %}
...
Tại tab **Participants**, click nút **Import**.
{% endstep %}

{% step %}
## Import danh sách

1. Click **Browse file** để chọn file danh sách học viên từ máy tính.
2. Click **Import** để bắt đầu xử lý.
{% endstep %}

{% step %}
## Xem kết quả import

Sau khi import xong, hệ thống hiển thị bảng kết quả:
...
(Tùy chọn) Click **View log** để xem chi tiết các dòng bị lỗi và nguyên nhân.
{% endstep %}
{% endstepper %}
...
Tại tab **Participants**, tìm học viên cần xóa (Source = Manually).
{% endstep %}
...
Xác nhận trong hộp thoại popup → Click **Yes**.
{% endstep %}
{% endstepper %}
...
#### Xem danh sách tổng hợp & chi tiết kết quả học viên

**Kết quả:** Xem được toàn bộ học viên đã tham gia kiểm tra đầu vào trên tất cả Program và bài kiểm tra.

{% stepper %}
{% step %}
## Mở danh sách tổng hợp

Vào **Menu → Entrance Test → Participants**.
{% endstep %}
...
{% step %}
## Tìm kiếm, lọc và xem tổng quan kết quả

1. (Tùy chọn) Tìm kiếm và lọc theo: **Tên học viên**, **Program**, **Subject**, **Entrance Test**, **Sort by**, **Từ ngày / Đến ngày**.
2. Xem tổng quan kết quả: click vào tên học viên để mở panel bên phải hiển thị:
